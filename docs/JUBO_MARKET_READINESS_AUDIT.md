...
| 4 | **Invite email flow** | 🟡 **Implemented, held for review (PR 4):** provider-agnostic email seam (`lib/email`) with a Resend adapter; `inviteMember`/`refreshInviteLink` send best-effort and return `emailSent`; UI shows sent-vs-link-only; copy-link fallback kept. **No required env, no new dependency, no schema, no auth/membership change** — inert until `RESEND_API_KEY` + `EMAIL_FROM` are set. Detail in `docs/JUBO_PR4_INVITE_EMAIL_FLOW.md` | Email provider = **external integration — gated**, held for review |
| 5 | **Billing/seat model foundation** | 🟡 **Foundation built, held for review (PR 5):** migration adds plan_type/seat_limit/billing_status/Stripe-ID/trial/period columns to `organizations` (seat_limit NULL = unlimited → no existing-org lockout); seat enforcement at both choke points (`accept_invitation` DB backstop + `inviteMember` friendly block), inert until a limit is set; read-only Plan & billing card (admin). **No Stripe code, no charging, no required env.** Rehearsed locally (8/8). Production SQL gated — runbook in `docs/JUBO_PR5_BILLING_SEAT_MODEL.md`. **Gated follow-ups:** Stripe checkout+webhook, suspension enforcement, self-serve billing | **Schema + payment-adjacent — held for review** |
| 6 | **Error monitoring / support ops** | 🟡 **6A held for review — zero-config, safe to merge:** (a) **Error boundaries: RESOLVED** — `global-error.tsx` + `(app)/error.tsx` show friendly crash pages, active immediately, no setup. (b) **Monitoring: code present, INACTIVE** — dependency-free Sentry-compatible reporter wired into `onRequestError` + boundaries, fully inert until `SENTRY_DSN`/`NEXT_PUBLIC_SENTRY_DSN` are set. (c) **Password reset: code present, DORMANT** — full Supabase flow implemented but unlinked + not proxy-allow-listed, activates later with a Supabase redirect-URL + a tiny re-expose PR. No required env, no Supabase change, no new dependency, no schema. Detail in `docs/JUBO_PR6A_MONITORING_PASSWORD_RESET.md`. **Still open (6B):** alert routing, vendor feedback visibility, support runbooks, audit-log table (schema), CI | Held for review |
| 7 | **Data export/offboarding** | 🟡 **Foundation done, held for review (PR 7):** owner/admin-only CSV export (Records, Record fields, Notes & tasks) from Settings → Organization — org-scoped via RLS + explicit filters, no service-role, no secrets, no mutation, CSV-injection-safe; plus a manual offboarding runbook. No schema. **Gated follow-ups:** SMS/message-body export, self-serve deletion/retention automation, bundled/streamed download. Detail in `docs/JUBO_PR7_DATA_EXPORT_OFFBOARDING.md` | New admin data surface — held for review; deletion path stays **destructive-gated** |
