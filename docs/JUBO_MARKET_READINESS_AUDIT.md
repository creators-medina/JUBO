...
| 6 | **Error monitoring / support ops** | 🟡 **6A done, held for review (PR 6A):** dependency-free Sentry-compatible error reporter wired into `onRequestError` + `global-error.tsx` + `(app)/error.tsx`; Supabase password-reset flow (forgot/reset pages + login link + proxy allow-list). No required env, no new dependency, no schema — inert until `SENTRY_DSN`/`NEXT_PUBLIC_SENTRY_DSN` set; reset needs a one-time Supabase redirect-URL/email setup. Detail in `docs/JUBO_PR6A_MONITORING_PASSWORD_RESET.md`. **Still open (6B):** alert routing, vendor feedback visibility, support runbooks, audit-log table (schema), CI | Monitoring = external service + **auth-touching** — held for review |
